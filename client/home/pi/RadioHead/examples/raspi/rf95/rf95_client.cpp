...
int main (int argc, const char* argv[] )
{
  ...
  if (!rf95.init()) {
    ...
    const char* cmd = "mosquitto_sub -C 1 -h serveur.iot.com -p 8883 -t /esp8266 --cafile /home/pi/certifs/ecc.ca.cert.pem --cert /home/pi/certifs/ecc.serveur.pem --key /home/pi/certifs/ecc.key.pem -u pi -P raspberry";
    
    const char* begincmd = "echo \"";
    const char* endcmd = "\" | openssl enc -base64 -e -aes-256-cbc -salt -pass pass:LovePFBplease20/20UwU -pbkdf2";
    
    char* cmd2 = (char*) malloc( sizeof(char)*(DATA_SIZE*2));
    
    char* data = (char*) malloc(sizeof(char)*DATA_SIZE);
    char* json = (char*) malloc( sizeof(char)*(DATA_SIZE*2));
    ...
    while (!force_exit) {
      ...
      if ( millis() - last_millis > 5000 ) {
        ...
        strcpy(cmd2,begincmd);
        strcat(cmd2,json);
        strcat(cmd2,endcmd);

        std::shared_ptr<FILE> pipe2(popen(cmd2, "r"), pclose);
        fgets(data, DATA_SIZE, pipe2.get());
        data[strlen(data) - 1] = '\0';
